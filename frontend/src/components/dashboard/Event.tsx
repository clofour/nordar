...
import { putApiEventSetOnetimeInstanceStateOnetimeEventId, putApiEventSetRecurringInstanceStateRecurringEventIdEventOccurence, useGetApiEventGetOnetimeInstanceStateOnetimeEventId, useGetApiEventGetRecurringInstanceStateRecurringEventIdEventOccurence } from "@/api/endpoints/event/event";
...
export default function Event({ event }: EventProps) {
    ...
    const { data: response, error, isLoading, mutate } = event.type == "recurring" && "recurringEventId" in event && "recurrenceId" in event ? useGetApiEventGetRecurringInstanceStateRecurringEventIdEventOccurence(event.recurringEventId, new Date(event.recurrenceId).toISOString()) : useGetApiEventGetOnetimeInstanceStateOnetimeEventId(event.id);
    useEffect(() => {
        if (response?.data) {
    ...
    const onChange = async (checkBoxEvent: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (event.type == "onetime") {
            response = await putApiEventSetOnetimeInstanceStateOnetimeEventId(
                event.id,
                requestData
            );
